...
class SQLite : public Connector {
 ...
 public:
    explicit SQLite(const QString &name, bool foreignKeysActivated = true,
                    bool test = false);
    inline auto areForeignKeysActivated() const;
    inline auto isTest() const;
    inline auto connectionName() const -> QString override;
...
auto SQLite::areForeignKeysActivated() const {
    return this->foreignKeysActivated;
}

auto SQLite::isTest() const {
    return this->test;
}

auto SQLite::connectionName() const -> QString {
    return this->getName() + FILE_EXTENSION;
}

auto SQLite::driverName() const -> QString {
    return "QSQLITE";
}
